...
def render_duel_state(duel, console: Console = None):
    ...
    for c in claims:
        ...
        claims_table.add_row(
            Text(icon, style=color), Text(f"[{c.index + 1}] {c.statement}", style=color)
        )

    console.print(
        Panel(
            claims_table,
            title="[bold]CLAIMS[/bold]",
            border_style="cyan",
            box=box.ROUNDED,
        )
    )
...
def render_reveal(reveal: dict, console: Console = None):
    ...
    if _gentle_mode:
        ...
        bars = {
            0: "[green]●●●●●[/green]",
            1: "[green]●●●●[/green][dim]○[/dim]",
            2: "[yellow]●●●[/yellow][dim]○○[/dim]",
            3: "[red]●[/red][dim]○○○○[/dim]",
        }
    ...
    if claims:
        ...
        for c in claims:
            ...
            table.add_row(
                str(c.index + 1), c.claim_type, c.statement, Text(f"{icon} {status}", style=color)
            )
        console.print(table)
    ...
    if history:
        ...
        for i, snap in enumerate(history):
            ...
            for e in snap.errors_at_time[:2]:
                sev_map = {1: "dim", 2: "yellow", 3: "red"}
                sev_color = sev_map.get(e.severity, "red")
                err_msg = (
                    f"      [{sev_color}]└─ {e.type} → claim {e.claim_index + 1}[/{sev_color}]"
                )
    ...
    console.print(
        Panel(verdict_text, title="[bold]VERDICT[/bold]", border_style=color, box=box.DOUBLE)
    )
